 ...
 */
class Base64MimeTypes implements Rule
{
    // Use case example
    // $request->validate([
    //     'base64_photos.*' => [new Base64MimeTypes('mimetypes=video/avi,video/mpeg,video/quicktime')],
    // ]);
    ...
    */
    ...
    public function passes($attribute, $value)
    {
        ...
        $file_ext = mime_content_type($value);

        // Get supplied_constraints
        $constraints = $this->supplied_constraints;

        // Check available user defined constrains
        if (!empty($constraints['mimetypes']) && !in_array($file_ext, $constraints['mimetypes'], true) ) {
            $this->appendToMessageBag([':attribute file under validation must match one of the given MIME types: '.implode(", ",$constraints['mimetypes'])]);
        }
    ...
    {
        return ['base_64_mime_types' =>  $this->getFromMessageBag()];
    }
}
